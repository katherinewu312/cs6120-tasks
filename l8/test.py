...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Benchmarking script with optimized and unoptimized runs."
    )
    parser.add_argument(
        "--runs", type=int, default=1, help="Number of runs for each benchmark"
    )
    ...
    for c_file in benchmark_dir.glob("*.c"):
        ...
        ll_command = (
            f"{clang} -S -emit-llvm -O0 -Xclang -disable-O0-optnone {c_file} -o a.ll"
        )
        ...
        optimized_time = measure_execution_time("./a_opt.out", num_runs=args.runs)
    ...
    print("\nResults:")
    print(
        f"{'Benchmark':<20}{'Original Time (s)':<20}{'Optimized Time (s)':<20}{'Improvement (%)':<15}"
    )
    improvements = []
    for benchmark, original_time, optimized_time in results:
        ...
        print(
            f"{benchmark:<20}{original_time:<20.4f}{optimized_time:<20.4f}{improvement:<15.2f}"
        )
